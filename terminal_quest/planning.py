...
class PlanStep:
    ...
    The current terminal game stores text commands. A future picture-based UI can
    map blocks onto the same raw command shape before execution.
    """
...
class ExecutionPlan:
    ...
    def add(self, raw: str, *, source: str = "text") -> PlanStep:
        cleaned = raw.strip()
        if not cleaned:
            raise ValueError("A plan step needs a command to run.")
