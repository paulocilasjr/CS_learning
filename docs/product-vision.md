...
type a command
  ...
  -> combine operations
  -> express decisions and repetition as programs
  -> debug and test behavior
  -> manage change over time
  -> solve unfamiliar computational problems
```
...
Plan-and-Run is the shared backbone for that reasoning. In the younger game, the child arranges picture steps and watches BB-8 carry them out. In the terminal game, those steps become shell commands. Later missions turn the same structure into code: plan the operations, run them, observe the result, and revise the plan when reality differs from intent.
